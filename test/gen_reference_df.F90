!
! Density-fitting reference generator: two- and three-centre Coulomb.
!
...
!   BLOCK2C <i> <j> <di> <dj>            / <di*dj doubles>
!   BLOCK3C <i> <j> <k> <di> <dj> <dk>   / <di*dj*dk doubles>
!   END
!
! %25.17e round-trips IEEE double exactly.
!
program gen_reference_df
   ...
   use cint_3c2e, only: int3c2e_cart, int2c2e_cart
   implicit none
   ...
   open (newunit=u, file='reference_df.dat', status='replace', action='write')
   ...
   write (u, '(a)') 'END'
   close (u)
   print '(a)', 'wrote reference_df.dat'
...
contains
   ...
   subroutine dump_system(u, isys, atm, bas, env, natm, nbas, nenv, ws)
      ...
      call dump_2c(u, atm, bas, env, natm, nbas, ws)
      call dump_3c(u, atm, bas, env, natm, nbas, ws)
      di = 0
   end subroutine dump_system

   subroutine dump_2c(u, atm, bas, env, natm, nbas, ws)
      integer, intent(in) :: u, natm, nbas
      integer, intent(in) :: atm(0:), bas(0:)
      real(dp), intent(in) :: env(0:)
      type(cint_ws), intent(inout) :: ws
      ...
      do j = 0, nbas - 1
         ...
         shls(0) = i; shls(1) = j
         dims(0) = di; dims(1) = dj
         hv = int2c2e_cart(buf, dims, shls, atm, natm, bas, nbas, env, ws)
         if (hv) then
            write (u, '(a,4(1x,i0))') 'BLOCK2C', i, j, di, dj
            write (u, '(es25.17)') (buf(m), m=1, n)
         end if
         deallocate (buf)
      end do
      end do
   end subroutine dump_2c

   subroutine dump_3c(u, atm, bas, env, natm, nbas, ws)
      integer, intent(in) :: u, natm, nbas
      integer, intent(in) :: atm(0:), bas(0:)
      real(dp), intent(in) :: env(0:)
      type(cint_ws), intent(inout) :: ws
      integer :: i, j, k, di, dj, dk, n, m, shls(0:2), dims(0:2)
      real(dp), allocatable :: buf(:)
      logical :: hv
      ...
      do k = 0, nbas - 1
         di = cint_cgto_cart(i, bas); dj = cint_cgto_cart(j, bas)
         dk = cint_cgto_cart(k, bas)
         n = di*dj*dk
         allocate (buf(n)); buf = 0.0_dp
         shls(0) = i; shls(1) = j; shls(2) = k
         dims(0) = di; dims(1) = dj; dims(2) = dk
         hv = int3c2e_cart(buf, dims, shls, atm, natm, bas, nbas, env, ws)
         if (hv) then
            write (u, '(a,6(1x,i0))') 'BLOCK3C', i, j, k, di, dj, dk
            write (u, '(es25.17)') (buf(m), m=1, n)
         end if
         deallocate (buf)
      end do
   ...
   end subroutine dump_3c

end program gen_reference_df
