...
module trc_pc_k1120
   ...
   implicit none
   private
   public :: pc1120, pcs1120

   real(dp), parameter :: TWO_PI_2_5 = 34.986836655249725_dp

contains

   !> (11|20) driver.  The `do concurrent` lives here and the
   ...
   subroutine pc1120(lo, hi, nseg, sOff, sA, sNB, sOA, sOB, sD, &
      ...
      do concurrent(i=1:nr)
         call pci1120(g0 + (i - 1)*nranks, lo, hi, nseg, sOff, sA, sNB, sOA, sOB, sD, &
                       npair, sp_i, sp_j, sp_q, thresh, pcut, jfac, kfac, dsh, nbas, npp, nao, sh_l, ao_off, &
   ...
   end subroutine pc1120

   pure subroutine pci1120(gt, lo, hi, nseg, sOff, sA, sNB, sOA, sOB, sD, &
                      npair, sp_i, sp_j, sp_q, thresh, pcut, jfac, kfac, dsh, nbas, npp, nao, sh_l, ao_off, &
                      pp_off, pp_n, pp_p, pp_r, pp_ra, pp_rb, pp_c, pp_cs, pp_ki, pp_kj, ncoltot, ncoef, ps_np, ps_ncol, ps_soff, ps_coff, col_ao, ps_coef, col_sh, nshc, nqc, q_col, dsh_c, &
                      ndens, dmat, jmat)
      !$acc routine seq
      ...
      ! locate the segment
         ...
         ncd_c = min(1, nccd - cd0 + 1)
            ...
            if (nqc > 1) then
               ...
               do qcd = 1, ncd_c
                  qchunk = max(qchunk, &
                     cb1120(ab0 + qab - 1, cd0 + qcd - 1, nca, nccl, si, sj, sk, sl, nbas, ncoltot, ps_soff, col_sh, nshc, nqc, q_col, dsh_c))
               end do
               end do
         ...
         if (nqc > 1) then
            if (cb1120(iabc, icdc, nca, nccl, si, sj, sk, sl, nbas, ncoltot, ps_soff, col_sh, nshc, nqc, q_col, dsh_c) <= thresh) ok = .false.
         end if
         if (.not. ok) cycle
   ...
   pure real(dp) function cb1120(iab_, icd_, nca, nccl, si, sj, sk, sl, &
