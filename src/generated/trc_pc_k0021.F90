...
module trc_pc_k0021
   ...
   implicit none
   private
   public :: pc0021, pcs0021

   real(dp), parameter :: TWO_PI_2_5 = 34.986836655249725_dp

contains

   !> (00|21) driver.  The `do concurrent` lives here and the
   ...
   subroutine pc0021(lo, hi, nseg, sOff, sA, sNB, sOA, sOB, sD, &
      ...
      do concurrent(i=1:nr)
         call pci0021(g0 + (i - 1)*nranks, lo, hi, nseg, sOff, sA, sNB, sOA, sOB, sD, &
                       npair, sp_i, sp_j, sp_q, thresh, pcut, jfac, kfac, dsh, nbas, npp, nao, sh_l, ao_off, &
   ...
   end subroutine pc0021

   pure subroutine pci0021(gt, lo, hi, nseg, sOff, sA, sNB, sOA, sOB, sD, &
                      npair, sp_i, sp_j, sp_q, thresh, pcut, jfac, kfac, dsh, nbas, npp, nao, sh_l, ao_off, &
                      pp_off, pp_n, pp_p, pp_r, pp_ra, pp_rb, pp_c, pp_cs, pp_ki, pp_kj, ncoltot, ncoef, ps_np, ps_ncol, ps_soff, ps_coff, col_ao, ps_coef, col_sh, nshc, nqc, q_col, dsh_c, &
                      ndens, dmat, jmat)
      !$acc routine seq
      ...
      ! locate the segment
         ...
         ncd_c = min(1, nccd - cd0 + 1)
            ...
            if (nqc > 1) then
               ...
               do qcd = 1, ncd_c
                  qchunk = max(qchunk, &
                     cb0021(ab0 + qab - 1, cd0 + qcd - 1, nca, nccl, si, sj, sk, sl, nbas, ncoltot, ps_soff, col_sh, nshc, nqc, q_col, dsh_c))
               end do
               end do
         ...
         if (nqc > 1) then
            if (cb0021(iabc, icdc, nca, nccl, si, sj, sk, sl, nbas, ncoltot, ps_soff, col_sh, nshc, nqc, q_col, dsh_c) <= thresh) ok = .false.
         end if
         if (.not. ok) cycle
   ...
   pure real(dp) function cb0021(iab_, icd_, nca, nccl, si, sj, sk, sl, &
   ...
   subroutine pcs0021(lo, hi, nseg, sOff, sA, sNB, sOA, sOB, sD, &
                      npair, sp_i, sp_j, sp_q, thresh, pcut, jfac, kfac, dsh, nbas, npp, nao, sh_l, ao_off, &
                      pp_off, pp_n, pp_p, pp_r, pp_ra, pp_rb, pp_c, pp_cs, pp_ki, pp_kj, ncoltot, ncoef, ps_np, ps_ncol, ps_soff, ps_coff, col_ao, ps_coef, col_sh, nshc, nqc, q_col, dsh_c, &
                      ndens, dmat, jmat, rank, nranks)
      integer,  intent(in)    :: lo, hi, nseg, npair, nbas, npp, nao
      ...
      if (g1 >= g0) nr = (g1 - g0)/nranks + 1
      do concurrent(i=1:nr)
         call pcsi0021(g0 + (i - 1)*nranks, lo, hi, nseg, sOff, sA, sNB, sOA, sOB, sD, &
                       npair, sp_i, sp_j, sp_q, thresh, pcut, jfac, kfac, dsh, nbas, npp, nao, sh_l, ao_off, &
                       pp_off, pp_n, pp_p, pp_r, pp_ra, pp_rb, pp_c, pp_cs, pp_ki, pp_kj, ncoltot, ncoef, ps_np, ps_ncol, ps_soff, ps_coff, col_ao, ps_coef, col_sh, nshc, nqc, q_col, dsh_c, ndens, dmat, jmat)
      end do
   end subroutine pcs0021

   pure subroutine pcsi0021(gt, lo, hi, nseg, sOff, sA, sNB, sOA, sOB, sD, &
                      npair, sp_i, sp_j, sp_q, thresh, pcut, jfac, kfac, dsh, nbas, npp, nao, sh_l, ao_off, &
                      pp_off, pp_n, pp_p, pp_r, pp_ra, pp_rb, pp_c, pp_cs, pp_ki, pp_kj, ncoltot, ncoef, ps_np, ps_ncol, ps_soff, ps_coff, col_ao, ps_coef, col_sh, nshc, nqc, q_col, dsh_c, &
                      ndens, dmat, jmat)
      !$acc routine seq
...
#endif
   end subroutine pcsi0021

end module trc_pc_k0021
