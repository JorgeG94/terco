...
contains
      ...
      ! locate the segment
         ...
         ncd_c = min(1, nccd - cd0 + 1)
            ...
            if (nqc > 1) then
               ...
               do qcd = 1, ncd_c
                  qchunk = max(qchunk, &
                     cb1212(ab0 + qab - 1, cd0 + qcd - 1, nca, nccl, si, sj, sk, sl, nbas, ncoltot, ps_soff, col_sh, nshc, nqc, q_col, dsh_c))
               end do
               end do
         ...
         if (nqc > 1) then
            if (cb1212(iabc, icdc, nca, nccl, si, sj, sk, sl, nbas, ncoltot, ps_soff, col_sh, nshc, nqc, q_col, dsh_c) <= thresh) ok = .false.
         end if
         if (.not. ok) cycle
   ...
   pure real(dp) function cb1212(iab_, icd_, nca, nccl, si, sj, sk, sl, &
